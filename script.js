...
// ================================================================
const SECRET_RULES = {
  // Jogador -> capitães com os quais ele NÃO pode jogar.
  blockedPlayersByCaptain: {
    "João Gui": ["Gordo", "Allan"]
  },

  // Jogadores que ficam escondidos até determinada equipe.
  reservedPlayersByTeam: {
    4: ["JP", "Moises", "Ygor", "JV", "Victor", "Davi"]
  },

  // Confrontos especiais: equipe -> lista de pares por rodada.
  specialPairsByTeam: {
    4: [
      ["Ygor", "Moises"],
      ["Victor", "Davi"],
  ...
  // 4ª rodada da Equipe 4: o algoritmo escolhe opções válidas
  // para completar a composição, usando apenas jogadores ainda disponíveis.
...
function reset(){
  ...
  const others=shuffle(CONFIG.captains.filter(c=>c!=="Bento"));
  state.captains=[others[0],others[1],others[2],"Bento",others[3],others[4]];
}
...
function team4Options(team){
  const round=state.choiceNumber;
  const pairs=SECRET_RULES.specialPairsByTeam[team.number] || [];

  // Nas três primeiras rodadas da Equipe 4, os confrontos são exatamente
  // os definidos pelo organizador. Os seis jogadores especiais só ficam
  // liberados a partir da Equipe 4; se sobrarem, voltam ao pool global
  // normalmente para as equipes 5 e 6.
  if(round<=pairs.length){
    const pair=pairs[round-1];
    let options=pair
      .map(name=>state.pool.find(p=>p.name===name))
      .filter(Boolean)
      .filter(p=>validForTeam(team,p));

    while(options.length<2){
      const candidates=availablePool().filter(p=>
        validForTeam(team,p) && !options.some(x=>x.name===p.name)
      );
      if(!candidates.length) break;
      options.push(pickRandom(candidates));
    }
    return shuffle(options).slice(0,2);
  }

  // Na 4ª rodada, completa a composição da equipe sem expor a lógica
  // de níveis na interface. Os jogadores não escolhidos nos confrontos
  // anteriores permanecem disponíveis e poderão aparecer nas equipes 5/6.
  return normalOptions(team);
}
function showDraftChoice(){
  const team=state.currentTeam;state.choiceNumber++;state.choiceRevealed=false;
  $("draft-team-label").textContent=`EQUIPE ${team.number} — ${team.captain}`;
  $("draft-title").textContent=`ESCOLHA ${state.choiceNumber} DE 4`;
  renderNeeds(team);
  state.choiceOptions=team.number===4?team4Options(team):normalOptions(team);
  const area=$("choices");area.innerHTML="";
  state.choiceOptions.forEach(p=>{
    const c=makeCard(p,{choice:true});c.classList.add("not-selectable");
    c.onclick=()=>choosePlayer(p,c);area.appendChild(c);
  });
  $("draft-message").textContent="Toque em REVELAR OPÇÕES para descobrir os dois jogadores.";
  $("btn-reveal-choices").classList.remove("hidden");show("draft");
}

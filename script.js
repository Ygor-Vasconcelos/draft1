...
const SECRET_RULES = {
  ...
  //
  // Eles ficam escondidos das equipes anteriores.
  // A partir da Equipe 2 ficam disponíveis.
  // ================================================================
  ...
  // 4ª rodada da Equipe 2 volta para a lógica normal.
  ...
  //
  // Esses jogadores NÃO entram no sorteio.
  // ================================================================
  fixedTeam6Players: [
    "Allan",
    "Murillo",
...
const CONFIG = {
  ...
  // Equipe 2 = Bento
  // Equipe 3 = Coutinho
  // Equipes 4, 5 e 6 = restantes aleatórios
  //
  // Allan e Murillo NÃO são mais capitães.
  // ================================================================
  captains: [
    "Pedro",
    "Abner",
...
function reset(){
  // ================================================================
  // 🔒 RETIRA OS JOGADORES DA EQUIPE 6 DO POOL
  // ================================================================
  const fixedNames =
...
function availablePool(){
  return state.pool.filter(p=>{
    ...
    // ============================================================
    // BLOQUEIO POR CAPITÃO
    // ============================================================
    const blockedCaptains=
      SECRET_RULES
        .blockedPlayersByCaptain[
          p.name
        ] || [];
    if(
      blockedCaptains.includes(
        captain
      )
    ){
      return false;
...
// OPÇÕES SECRETAS
//
// Esta função é usada pela Equipe 2.
// ================================================================
function team2Options(team){
  ...
  if(
    ...
    // Segurança:
    // se algum jogador não puder aparecer,
    // completa com uma opção válida.
    while(
      options.length<2
    ){
      const candidates=
...
function showDraftChoice(){
  ...
  renderNeeds(team);
  // ================================================================
  // EQUIPE 2 = LÓGICA SECRETA
  // TODAS AS OUTRAS = NORMAL
  // ================================================================
  state.choiceOptions=
    team.number===2
      ? team2Options(team)
